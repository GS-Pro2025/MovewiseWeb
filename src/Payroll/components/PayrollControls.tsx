...
export const PayrollControls: React.FC<PayrollControlsProps> = ({
  searchTerm, setSearchTerm, locationString,
  country, setCountry, state, setState, city, setCity,
  locationStep, setLocationStep, countries, states, cities, setCities, setStates,
  week, changeWeek, year, changeYear, weekInfo, loading,
  grouped, filteredOperators, weekDates, paymentStats, filteredTotalGrand,
}) => {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .week-dropdown-container { z-index: 1 !important; position: relative; }
      .week-dropdown { z-index: 1 !important; position: relative; top: 100%; left: 0; right: 0; }
    `;
  ...
  if (locationStep === "country") {
    options = countries;
    getOptionLabel = (o) => (typeof o === 'object' && 'country' in o ? o.country : '');
    label = t('payroll.controls.country');
    value = countries.find((c) => c.country === country) || null;
  } else if (locationStep === "state") {
    options = states;
    getOptionLabel = (o) => (typeof o === 'object' && 'name' in o ? o.name : '');
    label = t('payroll.controls.state');
    value = states.find((s) => s.name === state) || null;
  } else if (locationStep === "city") {
    options = cities;
    getOptionLabel = (o) => (typeof o === 'string' ? o : '');
    label = t('payroll.controls.city');
    value = city || null;
  }

  return (
    <div className="bg-white rounded-xl shadow-md border p-4 mb-4 week-dropdown-container"
      style={{ borderColor: COLORS.primary }} ref={dropdownRef}>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        {/* Week */}
        <div className="relative">
          <WeekPicker week={week} onWeekSelect={(selectedWeek) => {
        ...
        {/* Year */}
        <div className="relative">
          <YearPicker year={year} onYearSelect={changeYear} min={2020} max={new Date().getFullYear() + 2} />
        </div>
        ...
        <div className="relative">
          <label className="block text-xs font-bold mb-2" style={{ color: COLORS.primary }}>
            <div className="flex items-center gap-1.5">
              <Search size={14} />
              {t('payroll.controls.searchLabel')}
            </div>
          </label>
          <div className="relative">
            <Search size={14} className="absolute left-2 top-1/2 transform -translate-y-1/2 z-10"
              style={{ color: COLORS.primary }} />
            <input
              type="text"
              placeholder={t('payroll.controls.searchPlaceholder')}
              value={searchTerm}
            ...
            {searchTerm && (
              <button onClick={() => setSearchTerm("")}
                ...
                style={{ color: COLORS.error }} title={t('payroll.controls.clearSearch')}>
                <X size={12} />
              </button>
            )}
          </div>
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-2">
            <label className="block text-xs font-bold" style={{ color: COLORS.primary }}>
              <div className="flex items-center gap-1.5">
                <MapPin size={14} />
                {t('payroll.controls.locationLabel')}
              </div>
            </label>
            {locationString && (
              <button
                onClick={() => { setCountry(""); setState(""); setCity(""); setStates([]); setCities([]); setLocationStep("country"); }}
                ...
                title={t('payroll.controls.clearLocationTitle')}>
                <X size={10} />
                {t('payroll.controls.clearLocation')}
              </button>
            )}
          </div>
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...params} label={label}
                placeholder={t('payroll.controls.selectPlaceholder', { label: label.toLowerCase() })}
                size="small" className="bg-white rounded-lg"
                sx={{
          ...
          {locationString && (
            <div className="mt-2 p-1.5 bg-purple-50 rounded border border-purple-200">
              <p className="text-purple-700 font-medium text-xs font-bold">{locationString}</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="mt-4 pt-4 border-t" style={{ borderColor: COLORS.primary }}>
          <div className="flex flex-col md:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-3 rounded-lg p-3 border-2 shadow-sm" style={{ borderColor: COLORS.secondary }}>
              ...
              <div>
                <div className="text-xs font-bold uppercase tracking-wide" style={{ color: COLORS.primary }}>
                  {t('payroll.controls.period', { week, year })}
                </div>
            ...
            {!loading && grouped.length > 0 && (
              <PayrollExport
                operators={filteredOperators} weekInfo={weekInfo} weekDates={weekDates}
                week={week} year={year} location={locationString}
                paymentStats={paymentStats} totalGrand={filteredTotalGrand}
              />
            )}
          </div>
        </div>
