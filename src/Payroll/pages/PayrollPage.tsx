...
export default function PayrollPage() {
  ...
  const fetchData = async () => {
    try{
      ...
      res.data.forEach((d:AssignmentData)=>{
        ...
        if(!map.has(key)){
          map.set(key,{
            code:d.code,name:d.first_name,lastName:d.last_name,role:d.role,cost:d.salary,
            pay:payId??null,total:0,additionalBonuses:0,grandTotal:0,
            assignmentIds:[assignId],paymentIds:payId!=null?[payId]:[],
          });
        } else {
        ...
        row[day] = d.salary||0;
        row.total = Number(row.total||0) + Number(d.salary||0);
        row.additionalBonuses = Number(row.additionalBonuses||0) + Number(d.bonus||0);
  ...
  const totalGrand = useMemo(
    () => grouped.reduce((sum,r)=>sum+(r.grandTotal||0),0), [grouped]
  );
  ...
  const changeWeek=(e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    if(w>=1&&w<=53){ setWeek(w); setPage(1); }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        <div className="flex items-center">
          ...
          <input id="weekInput" type="number" min="1" max="53" value={week} onChange={changeWeek} className="border rounded px-2 py-1 w-20"/>
        </div>
        {weekInfo && <span className="text-sm text-gray-600">Period: {weekInfo.start_date} → {weekInfo.end_date}</span>}
      </div>
        ...
        <>
            ...
            <table className="min-w-full bg-white border border-gray-200">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  {weekdayKeys.map(day=><th key={day} className="py-2 px-4 border-b text-right">{day}</th>)}
              ...
              <tbody>
                {grouped.length ? grouped.map(r=>(
                  <tr key={r.code} className="hover:bg-gray-50 cursor-pointer" onClick={()=>setSelectedOperator(r)}>
                    ...
                    {weekdayKeys.map(day=><td key={day} className="py-2 px-4 border-b text-right">{r[day]?formatCurrency(r[day]!):'—'}</td>)}
                  ...
                  <tr><td colSpan={11} className="py-4 text-center text-gray-500">No data available</td></tr>
                )}
              </tbody>
            </table>
          </div>
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex gap-2">
              <button onClick={prev} disabled={page<=1} className={`px-3 py-1 border rounded ${page<=1?'bg-gray-100 text-gray-400':'bg-white text-blue-600 hover:bg-blue-50'}`}>Previous</button>
              <button onClick={next} disabled={page>=pages} className={`px-3 py-1 border rounded ${page>=pages?'bg-gray-100 text-gray-400':'bg-white text-blue-600 hover:bg-blue-50'}`}>Next</button>
            </div>
          </div>
        </>
      )}
