...
export const TableFilters: React.FC<TableFiltersProps> = ({
  ...
  if (locationStep === "country") {
    options = countries;
    getOptionLabel = (o: LocationOption) => (typeof o === 'object' && 'country' in o ? o.country : '');
    label = t('filters.country');
    value = countries.find((c) => c.country === country) || null;
  } else if (locationStep === "state") {
    options = states;
    getOptionLabel = (o: LocationOption) => (typeof o === 'object' && 'name' in o ? o.name : '');
  ...
  const finishedPct = weeklyStats.totalOrders > 0
    ? Math.round((weeklyStats.finishedOrders / weeklyStats.totalOrders) * 100) : 0;
  const pendingPct = weeklyStats.totalOrders > 0
    ? Math.round((weeklyStats.pendingOrders / weeklyStats.totalOrders) * 100) : 0;
  const inactivePct = weeklyStats.totalOrders > 0
    ? Math.round((weeklyStats.inactiveOrders / weeklyStats.totalOrders) * 100) : 0;
  ...
  return (
    <div data-statistics-panel="true">
      {/* Statistics Panel */}
      <div className="rounded-xl shadow-md border p-3 mb-2 transition-all duration-300" style={{ borderColor: COLORS.primary }}>
          ...
          <div className="p-3 rounded-lg border shadow-sm" style={{ borderColor: COLORS.primary }}>
            ...
            <div className="w-full bg-gray-200 rounded-lg h-3 overflow-hidden">
              <div className="h-full flex">
                <div className="transition-all duration-500" style={{ width: `${finishedPct}%`, backgroundColor: COLORS.success }} />
                <div className="transition-all duration-500" style={{ width: `${pendingPct}%`, backgroundColor: COLORS.secondary }} />
                <div className="transition-all duration-500" style={{ width: `${inactivePct}%`, backgroundColor: COLORS.error }} />
              </div>
            </div>
            <div className="flex justify-between text-xs font-semibold mt-2">
              <span className="flex items-center gap-1" style={{ color: COLORS.success }}>
                <CheckCircle size={12} />
                {t('filters.finished', { count: weeklyStats.finishedOrders, percent: finishedPct })}
              </span>
              <span className="flex items-center gap-1" style={{ color: COLORS.secondary }}>
                <Clock size={12} />
                {t('filters.pending', { count: weeklyStats.pendingOrders, percent: pendingPct })}
              </span>
              <span className="flex items-center gap-1" style={{ color: COLORS.error }}>
                <XCircle size={12} />
                {t('filters.inactive', { count: weeklyStats.inactiveOrders, percent: inactivePct })}
              </span>
            </div>
          </div>
        )}
      ...
      <div className="rounded-xl shadow-md border p-3 mb-3 week-dropdown-container transition-all duration-300" style={{ borderColor: COLORS.primary }} ref={dropdownRef}>
          ...
          <div className="lg:col-span-2 relative">
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                <Search size={12} className="absolute left-2 top-1/2 transform -translate-y-1/2 z-10" style={{ color: COLORS.primary }} />
                <input
                  type="text"
                  value={globalSearch}
                  ...
                  style={{ borderColor: COLORS.primary, backgroundColor: 'white', color: COLORS.primary }}
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 flex items-center gap-1 ${
                  globalSearchLoading || !globalSearch.trim() ? 'cursor-not-allowed' : 'text-white hover:shadow-md'
                }`}
                style={{
              ...
              >
                <Search size={12} className={globalSearchLoading ? 'animate-spin' : ''} />
              </button>
            </div>
          </div>
          ...
          {!isGlobalSearchActive && (
            <>
              ...
              <div className="relative">
                <YearPicker year={year} onYearSelect={onYearChange} min={2015} max={new Date().getFullYear() + 2} />
                <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full" style={{ backgroundColor: year ? COLORS.success : COLORS.error }} />
              </div>

              {/* Week */}
              <div className="relative">
                <WeekPicker week={week} onWeekSelect={onWeekChange} min={1} max={53} />
                <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full" style={{ backgroundColor: week ? COLORS.success : COLORS.error }} />
              </div>
              ...
              <div className="relative">
                ...
                <Autocomplete
                  ...
                  onChange={(_, newValue) => onWeekdayChange(newValue || '')}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      ...
                      sx={{
                        '& .MuiOutlinedInput-root': { borderRadius: '0.5rem', backgroundColor: 'white', fontSize: '0.875rem' },
                ...
                />
                <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full" style={{ backgroundColor: weekdayFilter ? COLORS.success : COLORS.error }} />
              </div>
              ...
              <div className="relative">
                  ...
                  {locationString && (
                    <button
                      onClick={() => {
                        ...
                        setStates([]); setCities([]); setLocationStep("country");
                      }}
                ...
                <Autocomplete
                  ...
                  onChange={(_, newValue) => {
                    if (newValue === null) {
                      ...
                      if (locationStep === "country" && typeof newValue === 'object' && 'country' in newValue) setCountry(newValue.country);
                      else if (locationStep === "state" && typeof newValue === 'object' && 'name' in newValue) setState(newValue.name);
                      else if (locationStep === "city" && typeof newValue === 'string') setCity(newValue);
                    }
                  }}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label={label}
                      placeholder={t('filters.selectLocation', { label: label.toLowerCase() })}
                      size="small"
                      className="bg-white rounded-lg"
                      sx={{
                        '& .MuiOutlinedInput-root': { borderRadius: '0.5rem', backgroundColor: 'white', fontSize: '0.875rem' },
                ...
                <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full" style={{ backgroundColor: locationString ? COLORS.success : COLORS.error }} />
              </div>
              ...
              <div className="flex flex-col justify-end">
                <button
                  ...
                  style={{ borderColor: COLORS.secondary, backgroundColor: COLORS.secondary, color: 'white' }}
                >
                  <Calendar size={14} />
        ...
        {/* Period Display */}
        {!isGlobalSearchActive && (
          <div className="rounded-lg p-2 border flex items-center justify-between" style={{ borderColor: COLORS.primary }}>
            <div className="flex items-center space-x-2">
              <div className="p-1 rounded-lg" style={{ backgroundColor: COLORS.primary }}>
            ...
            <div className="flex items-center space-x-2">
              <div className="px-2 py-1 rounded-lg border font-bold text-xs" style={{ borderColor: COLORS.secondary, color: COLORS.primary }}>
                {weekRange.start}
              </div>
        ...
        {isGlobalSearchActive && (
          <div
            ...
            style={{ borderColor: COLORS.primary, background: 'linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%)' }}
          >
            <div className="flex items-center justify-between">
              ...
              <div
                ...
                style={{ borderColor: COLORS.primary, backgroundColor: COLORS.primary, color: 'white' }}
              >
        ...
        <div className="mt-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {isGlobalSearchActive && (
              <div className="px-2 py-1 rounded-full text-xs font-bold text-white border shadow-sm flex items-center gap-1"
                style={{ backgroundColor: COLORS.primary, borderColor: COLORS.primary }}>
                <Globe size={10} />
                {t('filters.globalLabel', { search: globalSearch.length > 20 ? globalSearch.substring(0, 20) + '...' : globalSearch })}
              </div>
            )}
            {!isGlobalSearchActive && week && (
              <div className="px-2 py-1 rounded-full text-xs font-bold text-[#0B2863] border shadow-sm flex items-center gap-1"
            ...
            {!isGlobalSearchActive && (!week && !weekdayFilter && !locationFilter) && (
              <div className="px-2 py-1 rounded-full text-xs font-semibold border flex items-center gap-1"
                style={{ backgroundColor: COLORS.grayBg, borderColor: '#d1d5db', color: COLORS.gray }}>
                <Activity size={10} />
