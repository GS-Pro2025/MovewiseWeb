...
const mapTableDataToCreateOrderModel = (order: TableData): unknown => ({
  ...
  job: order.job || 0,
  customer_factory:
    typeof order.customer_factory === "number" ? order.customer_factory : 0,
});
...
const OrdersTable: React.FC = () => {
  ...
  const [createExtraCostDialogOpen, setCreateExtraCostDialogOpen] =
    useState(false);
  const [selectedOrderForExtraCost, setSelectedOrderForExtraCost] =
  ...
  const [selectedOrderForTools, setSelectedOrderForTools] =
    useState<NormalizedTableData | null>(null);
  ...
  const [paymentOrder, setPaymentOrder] = useState<NormalizedTableData | null>(
    null
  );
  ...
  const [orderToInactivate, setOrderToInactivate] =
    useState<NormalizedTableData | null>(null);
  const [deleteAbsoluteDialogOpen, setDeleteAbsoluteDialogOpen] =
    useState(false);
  const [orderToDeleteAbsolute, setOrderToDeleteAbsolute] =
    useState<NormalizedTableData | null>(null);
  const [dispatchTicketDialogOpen, setDispatchTicketDialogOpen] =
    useState(false);
  const [dispatchTicketUrl, setDispatchTicketUrl] = useState<string | null>(
    null
  );
  const [assignOrderToCostFuelDialogOpen, setAssignOrderToCostFuelDialogOpen] =
    useState(false);
  ...
  const [globalSearchLoading, setGlobalSearchLoading] =
    useState<boolean>(false);
  const [refreshCostFuelsTrigger, setRefreshCostFuelsTrigger] = useState<
    string | null
  >(null);
  ...
  const loadData = useCallback(
    async (searchTerm?: string) => {
      try {
        ...
        const mappedData: NormalizedTableData[] = response.results.map(
          (item) => {
  ...
  const handleGlobalSearch = useCallback(async () => {
    ...
    try {
      ...
      enqueueSnackbar(`Search completed for "${globalSearch}"`, {
        variant: "success",
      });
    } catch (error) {
    ...
    if (isGlobalSearchActive) {
      ...
      if (locationString) {
        filtered = filtered.filter((item) => {
          const itemLocation = [item.country, item.state, item.city]
            .filter(Boolean)
            .join(", ");
          return itemLocation
            .toLowerCase()
  ...
  const confirmFinishOrder = async () => {
    ...
    try {
      ...
      enqueueSnackbar("Sorry there was an error finishing the order", {
        variant: "error",
      });
  ...
  const handleSaveEdit = async (key: string, order: UpdateOrderData) => {
    ...
    if (result.success) {
      ...
      enqueueSnackbar(
        `Sorry there was an error updating the order: ${result.errorMessage}`,
        { variant: "error" }
      );
    }
  ...
  const handleChangeOrder = (
    field: keyof UpdateOrderData | `person.${string}`,
    value: unknown
  ) => {
    ...
    if (field.startsWith("person.")) {
      ...
      setOrderToEdit({
        ...orderToEdit,
        person: { ...orderToEdit.person, [personField]: value },
      });
    } else {
  ...
  const handleViewDispatchTicket = (order: TableData) => {
    ...
    if (url) {
      ...
      enqueueSnackbar("No dispatch ticket available for this order.", {
        variant: "info",
      });
    }
  };
  ...
  return (
    <Box sx={{ width: "100%", height: "100%" }}>
        ...
        <DeleteOrderDialog
          ...
          onConfirm={async () => {
            ...
            if (result.success) {
              ...
              enqueueSnackbar(
                result.errorMessage || "Error inactivating order",
                { variant: "error" }
              );
            }
            setInactivateDialogOpen(false);
      ...
      {deleteAbsoluteDialogOpen && (
        <DeleteOrderDialog
          ...
          onConfirm={async () => {
            ...
            if (result.success) {
              enqueueSnackbar("Order deleted permanently", {
                variant: "success",
              });
              loadData();
            } else {
              enqueueSnackbar(result.errorMessage || "Error deleting order", {
                variant: "error",
              });
            }
            setDeleteAbsoluteDialogOpen(false);
      ...
      <AssignOrderToCostFuelDialog
        ...
        onCreateFuel={() => {
          setAssignOrderToCostFuelDialogOpen(false);
          setCreateCostFuelOpen(true);
        }}
      />

      <CreateCostFuelDialog
        open={createCostFuelOpen}
        onClose={() => setCreateCostFuelOpen(false)}
        onSuccess={() => {
          setCreateCostFuelOpen(false);
          // Vuelve al flujo de asignación con la misma orden
          setAssignOrderToCostFuelDialogOpen(true);
        }}
      />
